...
@pytest.fixture
def zstd(tar: pathlib.Path, tmp_path: pathlib.Path) -> pathlib.Path:
    zstd = tmp_path.joinpath("zstd")

    with zstandard.open(zstd, "wb") as zstdf:
        with open(tar, "rb") as bio:
            zstdf.write(bio.read())
...
def test_unpack_zstd(zstd: pathlib.Path, tmp_path: pathlib.Path) -> None:
    dest = tmp_path.joinpath("dest")
    archive.unpack(str(zstd), str(dest))
    assert dest.joinpath("hello.txt").read_text() == "hello world\n"
