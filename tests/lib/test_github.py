...
def test_generate_and_configure_ssh_keypair(tmp_path: str) -> None:
    ...
    with (
        patch("devenv.lib.github.home", home),
        patch("devenv.lib.github.fs.idempotent_add") as mock_idempotent_add,
        patch(
            "devenv.lib.github.os.path.exists", return_value=False
        ) as mock_exists,
        patch("devenv.lib.github.proc.run"),
...
def test_check_ssh_access_success() -> None:
    with (
        patch("devenv.lib.github.CI", False),
        patch("devenv.lib.github.check_ssh_authentication", return_value=True),
...
def test_check_ssh_access_success_ci(ssh: bool, sso: bool) -> None:
    with (
        patch("devenv.lib.github.CI", True),
        patch("devenv.lib.github.check_ssh_authentication", return_value=ssh),
...
def test_check_ssh_access_success_external_contributor(sso: bool) -> None:
    with (
        patch("devenv.lib.github.CI", False),
        patch("devenv.lib.github.EXTERNAL_CONTRIBUTOR", "1"),
        patch("devenv.lib.github.check_ssh_authentication", return_value=True),
        patch(
...
def test_add_to_known_hosts(tmp_path: str) -> None:
    with (
        patch("devenv.lib.github.home", tmp_path),
