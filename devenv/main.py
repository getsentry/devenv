...
def devenv(argv: Sequence[str], config_path: str) -> ExitCode:
    ...
    # load local commands from $code_root/.devenv/commands -- allows a user to override defaults
    modinfo_list = load_modules(
        f"{code_root}/.devenv/commands", devenv.commands
    )
    # load default commands from installed devenv
    modinfo_list.extend(
        load_modules(devenv.commands.__path__[0], devenv.commands)
    )
